...
export function SearchVisitaId() {
  ...
  const visitaSearch = btnIdSearch.addEventListener("click", async () => {
    ...
    try {
      const [visita, cliente, product, tecnico] =
      await Promise.all([
      ...
      <div>
        ...
        <p><strong>Fecha:</strong> ${new Date(visita.Fecha_Visita_Tecnica).toLocaleDateString() }</p>
        <p><strong>Tipo visita:</strong> ${visita.Tipo_Visita}</p>
        <p><strong>Descripción:</strong> ${visita.Descripcion_Visita_Tecnica}</p>
        <p><strong>CC Técnico:</strong> ${tecnico.Nombre_Usuario} ${tecnico.Pk_CC_Usuario}</p>
        <p><strong>Producto:</strong>${product.Nombre_Producto} ${product.Fk_Id_Producto}</p>
        <p><strong>Cuenta cliente:</strong> ${cliente.Fk_Numero_Cuenta_Cliente}</p>
        </div>
    `;
